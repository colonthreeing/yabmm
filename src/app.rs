...
impl cosmic::Application for AppModel {
    ...
    const APP_ID: &'static str = "dev.julies_stuff.YABMM";
    ...
    fn init(
        ...
        nav.insert()
            .text(fl!("quickopts"))
            .data::<Page>(Page::QuickOps)
            .icon(icon::from_name("user-home-symbolic"))
            .activate();

        nav.insert()
            .text(fl!("download_mods"))
            .data::<Page>(Page::InstallMods)
            .icon(icon::from_name("folder-download-symbolic"));
        ...
        let content: Element<_> = match self.nav.active_data::<Page>().unwrap() {
            Page::QuickOps => {
                let header = widget::column::with_capacity(2)
                    ...
                    .push(widget::text::body(fl!("welcome_small")))
                    .spacing(space_s);

                let section = cosmic::widget::settings::section()
                    .add(widget::button::text(fl!("launch")).on_press(Message::LaunchGame))
                    .add(
                        widget::button::text(fl!("about"))
